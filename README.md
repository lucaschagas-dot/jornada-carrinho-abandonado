...
| **Resumo preso na lateral**, com as seções abrindo no chevron e um lápis que volta para a etapa de origem | `ResumoVida` + `BarraEtapa` (modo `fixo`) | mesma navegação do Residencial: onde há resumo lateral, ele fica grudado no topo da tela e só o miolo rola. Duas diferenças que a jornada obrigou: o lápis some quando a seção é da etapa atual (no Residencial o resumo só existe no pagamento, aqui está em nove etapas), e o total continua sendo a mensalidade — o Seguro de Vida não tem prêmio anual parcelado, então o bloco "1x de X / ou até Nx de Y" não foi copiado |
| **Assistências, Beneficiários e Declaração de Saúde** como seções próprias do resumo | `ResumoVida` | cada uma tem um lápis com destino diferente, e as duas últimas já estavam no estado da simulação sem aparecer em lugar nenhum |
| **Capital segurado em três cards de escolha**, no lugar do slider contínuo | `VidaProduto` | proposta: a loja entrega um slider de R$ 1.000 em R$ 1.000 e deixa a pessoa decidir sozinha quanto contratar. Os três valores saem do que a própria loja calcula (36x a renda, dentro do teto da profissão) e o do meio vem marcado como recomendado — mesma leitura dos combos do Residencial e dos planos do Odonto. Quem quiser outro valor continua podendo: o slider ficou atrás de um link discreto |
